...
namespace Redcat.Amqp.Tests.Serialization
{
    ...
    public class AmqpDataReaderTests
    {
        ...
        [Test]
        [ExpectedException(typeof(NotSupportedException))]
        public void Read_Throws_Exception_For_Unknown_Descriptor_Type()
        {
            buffer.Write(DataTypeCodes.Descriptor, 0xfe, 3, 4, 56, 6);
            reader.Read(buffer);
        }

        [Test]
        public void Read_Returns_Correct_ULong_Descriptor_Value([ValueSource(nameof(GetReadULongTestData))]Tuple<ulong, byte[]> data)
        {
            buffer.Write(DataTypeCodes.Descriptor);
            buffer.Write(data.Item2);
            Descriptor value = reader.Read(buffer) as Descriptor;

            Assert.That(value.IsULong, Is.True);
            Assert.That(value.ULongValue, Is.EqualTo(data.Item1));
        }

        [Test]
        public void Read_Returns_Correct_String_Descriptor_Value([ValueSource(nameof(GetReadStringTestData))]Tuple<string, byte[]> data)
        {
            buffer.Write(DataTypeCodes.Descriptor);            
            buffer.Write(data.Item2);
            Descriptor value = reader.Read(buffer) as Descriptor;
            ...
            Assert.That(value.StringValue, Is.EqualTo(data.Item1));
        }
        ...
        private void VerifyReadMethod(object expectedValue, byte[] bytes)
        {
            ...
            object actualValue = reader.Read(buffer);
        ...
        private void VerifyGenericReadMethod<T>(T expectedValue, byte[] serializedData)
        {
            ...
            T actualValue = reader.Read<T>(buffer);
